...
Return value: If error encountered, returns None. If RETURN_SLICE is True,
    ...
    kwdefs = { 'transpose':False, 'conductor':None, 'overplot':False, \
               'block':'all','multiply':None, 'cbtitle':None, \
...
WARNING: Supplied slice orientation and normal for an OVERPLOT plot are not
    ...
    if not overplot:
        ...
        if polar and cntour != 2:
            twopi =  2.0*np.pi
            drg = yrp[1] - yrp[0]
            if abs(yrp[0]) < 1.e-5: yrp[0] = 0.0
            else:
                while yrp[0] < 0.0: yrp[0] += twopi
                while yrp[0] > twopi: yrp[0] -= twopi
            yrp[1] = min(yrp[0] + drg,twopi)
...
_plotvecdefs = dict(xrange=None, yrange=None, levels=None,
                    ...
                    cbar=None, c_colors=None, cmap=None,
                    return_slice=False, lw=None, charsize=None,
...
Arguments:
  ...
  cbar:         If set, color bar is to be plotted. Valid values are:
                  ...
                  [x0,y0,dx,dy], indicating overlay position on plot axes, in
                     axes-normalized units.
  cbtitle:      Title for color bar
  variable:     If true, the length of the drawn vectors is scaled
                proportionally to the magnitude of the vector field. Otherwise,
                the length of the vectors is constant. If not specified,
                defaults to false if `color' = 'mag', and true if not.
  setdefault:   If set, provided option values will be made default
  showtdefault: If set, current default option values will be printed
  unset:        If set, default option values will be reset to original values
...
Return value: If error encountered, returns None. If RETURN_SLICE is True,
    ...
    kwdefs = dict(transpose=False, conductor=None, overplot=False,
                  block='all',multiply=None, cbtitle=None, variable=None,
...
WARNING: Supplied slice orientation and normal for an OVERPLOT plot are not
    ...
    elif variable is None: variable = True
    ...
    if polar == 0:
        datlims = np.empty((2,2),dtype=float)
        datlims[:,0] = ax.get_xlim()
    ...
    if polar == 0:
        ...
        if xygrid:
            ...
            if delt[0] > delt[1]:
                xshape = np.array([ npmax, int(npmax*delt[1]/delt[0] + .499)]) 
            else:
    ...
    if polar != 0:
        hxlen = (xrr[1]-xrr[0])/xshape[0]
        ylen = (yrr[1]-yrr[0])/xshape[1]
        if not xygrid:
            f1 = 0.5/xshape[1]
        ...
        if polar == 0:
            ...
            if xygrid:
                ...
                if polar == 2:
                    ...
                    cs,sn = (np.cos(yg[j]), np.sin(yg[j]))
                    x,y = (xg[i]*cs,xg[i]*sn)
            # (Ex,Ey) are (Etheta,Er) in polar
            exy =  np.array([ex[i,j]*cs - ey[i,j]*sn, ex[i,j]*sn + ey[i,j]*cs])

        p0 =  np.array([x,y])
        if polar == 0:
            alp = hplen/np.sqrt(np.sum(np.square(exy*pixrate)))
            if variable: alp *= (emag[i,j]/emagmax)
            pdel = alp*exy
            p1 = p0 + pdel
            p2 = p0 - pdel
        elif polar == 2:
            if variable: alp = hxlen/emagmax
            else: alp = hxlen/emag[i,j]
            pdel = alp*exy
            p1 = p0 + pdel
            p2 = p0 - pdel
        else:
            ...
            pdel = alp*exy
            ptmp = np.array([p0 + pdel, p0 - pdel])
            ##print('po=1',p0,pdel,ptmp,x,y)
            p1,p2 = np.transpose( \
                        np.array([np.arctan2(ptmp[:,1],ptmp[:,0]),
...
Usage:
  plotpar(ds, [x, y] , [weight], [xrange=list], [yrange=list], [xlabel=str],
          ...
          [edgecolors=str], [w1=list, [w2=list, ...]] )
...
Arguments:
  ...
  x:          Direction for horizontal plot axis (integer). If negative,
              indicates attribute dimension. If polar is True, `x' must be
              positive.
  y:          Direction for vertical plot axis (integer). If negative,
              indicates attribute dimension. If polar is True, `y' must be
              positive.
  ...
  aspect:     Aspect ratio of plot
  polar:      If True, assume `x' and `y' are the polar coordinates r an theta,
              respectively, and plot the particles in polar coordinates. 
  ...
  conductor:  Conductor dataset (or list of datasets) to be overplotted.
              Must describe a valid conductor (some PFF Vertex datasets or
              pfmpl-2d quadlist3d dataset)
  color:      An integer or string. Strings are interpreted as names of
              matplotlib colors. Integers are mapped (modulo 8) to 'k'
              (black), 'r' (red), 'g' (green), 'b' (blue), 'c' (cyan),
              'm' (magenta), 'y' (yellow), and 'orange'. Ignored if WEIGHT
              option is supplied. Default: black
  cmap:       Matplotlib color map to be used.
  cbar:       If set, color bar is to be plotted. Valid values are:
                'h' or 'v', indicating horizontal or vertical placement
                [x0,y0,dx,dy], indicating overlay position on plot axes, in
...
Return value: If successful, returns number of particles plotted. Otherwise,
              None is returned'''
    ...
    if x == 0: xokay = False
        ...
        if -x > adim or pflag: xokay =False
        else: ix = -x - 1
    if y == 0: yokay = False
    elif y > 0:
    ...
    if polar == 0:  #FIXME for polar, x & y must be > 0
        ...
        # in matplotlib polar, x1,x2 == theta,r
        xd = xdata[iy,indo]
        yd = xdata[ix,indo]
    elif polar == 2:
        xd = xdata[ix,indo]*np.cos(xdata[iy,indo])
        yd = xdata[ix,indo]*np.sin(xdata[iy,indo])
    ...
    if ind is not None:
        ...
        if not overplot and yrange is None:
            if polar:
                yr = np.asarray([xd.min(),xd.max()])/np.pi
                yrng = np.asarray(utils.nice_bounds(yr[0],yr[1])[0:2])*np.pi
                print('yrx',yr,yrng)
                ax.set_xlim(yrng)
            else:
