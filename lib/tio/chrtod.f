      double precision function chrtod (c,ierr)
c
c***********************************************************************
c     $Id$
c     
...
c***********************************************************************
c
c     CHRTOD converts a character string to a DOUBLE PRECISION number.
c     It does nothing and returns the value 0.0D0 if the error flag
c     is non-zero.  If a syntax error is detected in the input string,
c     it returns the value 0.0D0, and error status IERR = -1.
c
...
c
        if (curint.eq.INT) then
          chrtod = chrtod*TEN + digit
        elseif (curint.eq.FRAC) then
          chrtod = chrtod + digit*tenfr
          tenfr  = tenfr/TEN
